...
function Feedback({ params }) {
    ...
    const { interviewId } = React.use(params); // Destructure interviewId directly from params
    const [feedbackList,setfeedbackList] =useState([])

    useEffect(() => {
        getFeedback();  // Call the function to get feedback when the component mounts
    }, [interviewId]);  // Add interviewId as a dependency to refetch data if it changes

    const getFeedback = async () => {
        try {
            const result = await db.select()
                .from(UserAnswer)
                .where(eq(UserAnswer.mockIdRef, interviewId))  // Corrected the query
    ...
    return (
        <div className='p-10'>
            ...
            <h2 className='font-bold text-2xl'>No Interview Feedback Record Found</h2>:
            <>
...

            {
                feedbackList&&feedbackList.map((item , index)=>{
                    ...
                    flex justify-between gap-7 w-full'>{item.question}
